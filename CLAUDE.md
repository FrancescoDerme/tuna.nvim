...
3. **Extend** — once parity with competitest.nvim is reached, add new features beyond what the original plugin had. Roadmap in `/home/tuna/.claude/plans/lively-hatching-llama.md`; per-feature manual-test steps in the local-only `test.md`. Workstreams:
   ...
   - [x] W2. stress testing, interactive problems, multiple-answer (via checker), multiple solution versions (`run_all`); plus Lua-function checker, scaffolding, and a mode-switcher menu
...
_(Phase 1 review complete. Phase 2 port complete — all modules ported. Phase 3 (Extend) in progress: W1 + stress testing done.)_
...
- **`checker.lua`** (Phase 3) — verdict layer. `judge(tc, checker, compare_method, callback)`: `"builtin"` delegates to `compare.lua`; an external testlib-style checker is invoked as `checker <input> <output> <answer>` (placeholders `$(INPUT)`/`$(OUTPUT)`/`$(ANSWER)`), exit 0 = correct, stderr/stdout = `tc.checker_message`. Async (callback-based) because the external path spawns a process. The runner resolves the `checker` config (string path or `{ exec, args }`) in `new()`
- **`stress.lua`** (Phase 3) — stress testing (`:Tuna stress [count]`). Reuses `runner.new()` for the solution's resolved compile/run commands + checker; compiles once, then loops: `generator(seed)` → input, runs the solution and the `reference` on it, judges via `checker.judge` (sol output vs reference output). First wrong-answer/crash/timeout is saved as a new testcase and the search stops. Config under `stress = { generator, reference, count, seed_arg }`
- **`interactive.lua`** (Phase 3) — interactive problems (`:Tuna interactive [n…]`). Spawns the solution and an `interactor` via `vim.uv.spawn` and cross-wires their stdio (sol.stdout→int.stdin, int.stdout→sol.stdin); the interactor is the verdict authority (exit 0 = CORRECT). The interactor receives the testcase input/answer through `$(INPUT)`/`$(ANSWER)` temp-file placeholders (only the exec is modifier-expanded; args stay raw). Per-testcase sequential, timeout-guarded; reuses `runner.new()` for solution resolution. Config under `interactive = { interactor }`
- **`multi.lua`** (Phase 3) — multiple solution *versions* (`:Tuna run_all`). Runs every sibling source file with the current file's extension (so they share its filetype/commands) against the shared testcases, compiling each and judging via the `checker`; reports a per-solution `correct/total` summary. Commands are resolved per file with `utils.eval_string` (no open buffer). Distinct from multiple-*answer* problems, which the `checker` already handles
- **`scaffold.lua`** (Phase 3) — `:Tuna scaffold <checker|generator|brute>`: writes a dependency-free C++ starter file into the problem directory and opens it. Built-in templates; filename/template overridable via `config.scaffold.{files,templates}`
- **`checker`/`stress`/`interactive`/`multi`/`scaffold`** are also reachable through the **mode-switcher menu** (`widgets.menu`): bare `:Tuna` (or `:Tuna menu`) opens a native chooser listing Run / Run-all / Stress / Interactive / Scaffold actions (precursor to the W5 dashboard)

...
- **`commands.lua`** — the `:Tuna <subcommand> [args…]` surface. `execute(fargs)` dispatches; `complete` gives context-aware tab-completion (subcommand names, then per-subcommand args for `run`/`convert`/`receive`/`scaffold`/`checker`). Handlers: `add_testcase`/`edit_testcase [n]`/`delete_testcase [n]` (wire the `widgets` editor/picker), `convert <files|single_file|directory>` (loads via auto-detect, clears all backends, rewrites to the target), `run [normal|all|stress|interactive] [args…]` — the **run-mode dispatcher**: a leading mode keyword switches the buffer's active mode (via `tools.set_mode`) and is consumed, else the stored mode is used; `M.dispatch_mode` routes to `run_testcases`/`multi`/`stress`/`interactive`. Also `run_no_compile [n…]`/`show_ui` (per-buffer `M.runners`, cleared on `BufUnload`), `checker [on|off|toggle]` (per-buffer special-judge toggle; drops the cached runner), `receive <testcases|problem|contest|persistently|status|stop>`, `scaffold <checker|generator|brute|interactor> [ext]`, `menu` (bare `:Tuna` opens the mode-switcher menu)
...
- **`tools.lua`** (Phase 3) — helper-program abstraction + per-buffer run state. `find(dir, role, cfg)` discovers a sibling helper source file by filename convention (`checker.*`/`gen.*`/`brute.*`/`interactor.*`, names from `tool_names`); `program(path, cfg)`/`checker_spec(path, cfg)` resolve a path into a runnable spec (`{ exec, args, compile?, compile_dir?, cwd }`) using the file's own filetype→compile/run commands; `prepare(spec, cb)` compiles the spec **once** and caches the result (queuing concurrent callers so parallel judges don't recompile/race). Also holds the per-buffer run state keyed by file path: `get_mode`/`set_mode` (`normal`/`all`/`stress`/`interactive`) and `checker_enabled`/`set_checker`
